...
module.exports = function(config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.createSession = function(provider, req) {
  ...
  this.generateJWT = function(user, tempUser, tempPass, tempExpires, permanent) {
    return new Promise((resolve, reject) => {
      let tokenid = uuidv4();
      let token = {
        jti: tokenid,
      ...
      if (!permanent) {
        token["exp"] = Math.floor(Date.now() / 1000) + (config.getItem("security.jwt.expires") || 900);
      }
      jwt.sign(token, config.getItem("security.jwt.secret"), (err, token) => {
        if (err) {
          reject(err);
        }
        resolve(token);
  ...
  this.refreshSession = function(req) {
    var user = req.user;
    var newSession;
    if (user.payload.dbExpires < Math.floor(Date.now() / 1000)) {
      ...
      return couchAuthDB.get("org.couchdb.user:" + user.payload.dbUser).then(function(tempUser) {
        ...
        newSession.token = jwt_;
        newSession.dbUser = user.payload.dbUser;
        newSession.dbPass = user.payload.dbPass;
        newSession.dbExpires = newToken.expires;
        newSession.user_id = user._id;
        newSession.roles = user.roles;
        // Inject the list of userDBs
