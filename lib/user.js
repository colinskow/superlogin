...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.forgotPassword = function(userOrEmail, req) {
    req = req || {};
    var user, token, tokenHash, query;
    if(emailUsername) {
      query = 'emailUsername';
    } else {
      query = EMAIL_REGEXP.test(userOrEmail) ? 'email' : 'username';
    }
    return userDB.query('auth/' + query, {key: userOrEmail, include_docs: true})
      .then(function(result) {
