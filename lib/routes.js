...
module.exports = function(config, router, passport, user) {
  ...
  router.post("/login", function(req, res, next) {
    ...
    // Success handler
    return user.createSession("local", req, true)
  ...
  router.post("/refresh",
    ...
    function(req, res, next) {
      return user.refreshSession(req)
        .then(function(mySession) {
          res.status(200).json(mySession);
        }, function(err) {
          return next(err);
  ...
  router.post("/logout-others",
    ...
    function(req, res, next) {
      ...
      user.logoutOthers(req.user, req.user.payload.dbUser)
        .then(function() {
    ...
    user.resetPassword(req.body, req)
      .then(function(currentUser) {
        if (config.getItem("security.loginOnPasswordReset")) {
          return user.createSession("local", req)
            .then(function(mySession) {
              res.status(200).json(mySession);
            }, function(err) {
              return next(err);
