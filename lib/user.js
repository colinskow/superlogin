...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  function addCustomUserDBs(approach, userdbList, userOpposedbList, customPrivateDBs, customSharedDBs, defaultPrivateDBs, defaultSharedDBs, type) {
        var dbs;
        var defaultOpposedDBs;
        if(approach!=='override' && approach!=='rename'){
            throw new Error("Invalid value " + approach  );
        }
        if (type === 'private') {
            dbs = customPrivateDBs;
            defaultOpposedDBs = defaultSharedDBs;
        } else {
            dbs = customSharedDBs;
            defaultOpposedDBs = defaultPrivateDBs;
        }
        dbs.forEach(function (nameDB) {
            var indesDb = defaultOpposedDBs.indexOf(nameDB);
            var indexOpposeDb = userOpposedbList.indexOf(nameDB);

            if (indesDb > -1 || indexOpposeDb > -1 ) {
                if(approach === 'override'){
                    if(indexOpposeDb > -1){
                        userOpposedbList.splice(indexOpposeDb,1);
                    }
                    if(indesDb >-1){
                        defaultOpposedDBs.splice(indesDb,1);
                    }
                }else{
                    var index;
                    if(type === 'private'){
                        index = customPrivateDBs.indexOf(nameDB);
                        if (index > -1) {
                            customPrivateDBs.splice(index, 1);
                        }
                        index = defaultPrivateDBs.indexOf(nameDB);
                        if(index > -1){
                            defaultPrivateDBs.splice(index);
                        }
                        index = userdbList.indexOf(nameDB);
                        if (index > -1) {
                            userdbList.splice(index,1);
                        }
                    }else{
                        index = customSharedDBs.indexOf(nameDB);
                        if (index > -1) {
                            customSharedDBs.splice(index, 1);
                        }
                        index = defaultSharedDBs.indexOf(nameDB);
                        if(index > -1){
                            defaultSharedDBs.splice(index);
                        }
                        index = userdbList.indexOf(nameDB);
                        if (index > -1) {
                            userdbList.splice(index,1);
                    ...
                    nameDB=nameDB+"_"+type;
                }
            }
            if(!userdbList.includes(nameDB)) {
                userdbList.push(nameDB);
            }
        });
        if(type === 'private'){
            defaultSharedDBs.concat(customSharedDBs).filter(function (element) {
                if(!userOpposedbList.includes(element) && !userOpposedbList.includes(element+"_shared")) {
                    userOpposedbList.push(element);
                }
            });
        }else{
            defaultPrivateDBs.concat(customPrivateDBs).filter(function (element) {
                if(!userOpposedbList.includes(element) && !userOpposedbList.includes(element+"_private")) {
                    userOpposedbList.push(element);
                }
            });
        }
  ...
  function addUserDBs(newUser) {
        ...
        var customUserDBs = config.getItem('userDBs.customDBs');
        if (!Array.isArray(customUserDBs)) {
            customUserDBs = [];
        }
        ...
        var userPrivateDBs= [];
        var userSharedDBs = [];
        var roleUserList = newUser.roles;
        var customrol = false;
        customUserDBs.forEach(function (element) {
            if(roleUserList.includes(element.roles)) {
                var customPrivateDBs = element.DBs.private || [];
                var customSharedDBs = element.DBs.shared || [];
                var approach = element.approach || 'override';
                addCustomUserDBs(approach, userPrivateDBs, userSharedDBs, customPrivateDBs, customSharedDBs, defaultPrivateDBs, defaultSharedDBs, 'private');
                addCustomUserDBs(approach, userSharedDBs, userPrivateDBs, customPrivateDBs, customSharedDBs, defaultPrivateDBs, defaultSharedDBs, 'shared');
                customrol = true;
            }
        });
        if(!customrol){
            userPrivateDBs = defaultPrivateDBs;
            userSharedDBs = defaultSharedDBs;
        }

        console.log(userPrivateDBs);
        console.log(userSharedDBs);

        processUserDBs(userPrivateDBs, 'private');
        processUserDBs(userSharedDBs, 'shared');
