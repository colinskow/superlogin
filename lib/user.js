...
module.exports = function(config, userDB, couchAuthDB, mailer, emitter) {
  ...
  this.validateUsername = function(username) {
    ...
    return userDB.query('auth/username', { key: username }).then(function(result) {
  ...
  this.validateEmail = function(email) {
    ...
    return userDB.query('auth/email', { key: email }).then(function(result) {
      if (result.rows.length === 0) {
        // Pass!
        return Promise.resolve();
      } else {
  ...
  this.createSession = function(user_id, provider, req) {
    ...
    return userDB
      ...
      .then(function() {
        ...
        if (config.getItem('local.sendNameAndUUID')) {
          if (user.name) {
  ...
  this.forgotPassword = function(email, req) {
    ...
    return userDB
      ...
      .then(function(result) {
        ...
        token = util.URLSafeUUID();
        if (config.getItem('local.tokenLengthOnReset')) {
          token = token.substring(0, config.getItem('local.tokenLengthOnReset'));
        }
