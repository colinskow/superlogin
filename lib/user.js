...
module.exports = function (config, userDB, couchAuthDB, mailer, emitter) {
  ...
  var tokenLife = config.getItem('tokenLife') || 86400;
  // Session token life
  var sessionLife = config.getItem('sessionLife') || 86400;
  ...
  this.get = function (login) {
    ...
    return new BPromise(function (resolve, reject) {
      userDB.query('auth/' + query, {key: login, include_docs: true})
          .then(function (results) {
            if (results.rows.length > 0) {
              return resolve(results.rows[0].doc);
            } else {
              return resolve(null);
            }
          });
  ...
  this.create = function (form, req) {
    ...
    return user.process()
        .then(function (result) {
          ...
          // an welcome event to share the login and unhashed password to the user.
          emitter.emit('welcome', newUser, 'local');
    ...
    return BPromise.resolve()
        ...
        .then(function() {
          if(action === 'signup') {
            ...
            // a welcome event.
            emitter.emit('welcome', user, provider);
          }
          return BPromise.resolve(user);
        });
  };
  ...
  this.logoutUser = function(user_id) {
    var user;
    // var sessions;
    return userDB.get(user_id)
        .then(function(record) {
  ...
  function addUserDBs(newUser) {
    ...
    var processUserDBs = function(dbList, type) {
      dbList.forEach(function(userDBName) {
        ...
        promises.push(
            dbAuth.addUserDB(newUser, userDBName, dbConfig.designDocs, type, dbConfig.permissions)
                .then(function(finalDBName) {
