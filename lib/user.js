    ...
    return passwordResetForm.validate()
      ...
      .then(function (results) {
        ...
        if(!!form.email && user.email != form.email){
            return BPromise.reject({status: 400, error: 'Wrong Email'});
        }
  ...
  this.forgotPassword = function(email, req) {
    ...
    return userDB.query('auth/email', {key: email, include_docs: true})
      .then(function(result) {
        ...
        if(config.getItem('security.tokenPasswordResetType') == 'pin') {
            user.forgotPassword = {
                token:  util.fiveDigitPin(),
                issued: Date.now(),
                expires: Date.now() + (900 * 1000) //15 minutes expiration
            };
        }
        else {
            user.forgotPassword = {
    ...
    .then(function() {
        var template;
        if(config.getItem('security.tokenPasswordResetType') == 'pin') {
            template = 'forgotPasswordDigit';
        }
        else{
            template = 'forgotPassword';
        }
        return mailer.sendEmail(template, user.email || user.unverifiedEmail.email,
