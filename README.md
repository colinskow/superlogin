...
4. To update a custom field at some point in the future, use the `/change-profile` endpoint. Example:
  ...
  POST the form to `http://localhost:3000/auth/change-profile` with Authentication header and you should get the response `{"ok": true, "success": "Fields changed"}`.
...
##### `POST /change-profile`
Authentication required. Changes the user's profile custom fields.
