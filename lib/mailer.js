...
module.exports = function(config) {
  ...
  if(config.getItem('testMode.noEmail')) {
    ...
    var options = config.getItem('mailer.options');
    var provider = config.getItem('mailer.provider');
    if(provider === 'ses') {
      ...
      transporter = nodemailer.createTransport(customTransport(options));
    }
  } else {
  ...
  this.sendEmail = function(templateName, email, locals) {
    ...
    if(config.getItem('siteURL')) {
      locals.siteURL = config.getItem('siteURL');
    } else {
      locals.siteURL = '';
    }
